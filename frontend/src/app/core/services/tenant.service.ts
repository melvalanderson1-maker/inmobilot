...
export class TenantService {
  ...
  cargar(): Promise<void> {
    return new Promise((resolve) => {
      this.http.get<TenantConfig>(`${environment.apiUrl}/config/tenant`).subscribe({
        next: (cfg) => {
          this.config.set(cfg);
          this.aplicarTema(cfg);
          resolve();
        },
        error: () => resolve(), // si falla, la app sigue con valores por defecto del CSS
      });
    });
  }
  ...
  private aplicarTema(cfg: TenantConfig): void {
    ...
    document.documentElement.style.setProperty('--color-secundario', cfg.color_secundario);
